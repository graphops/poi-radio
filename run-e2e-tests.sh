...
run_test_with_timeout() {
    ...
    if timeout $timeout_value cargo run --bin integration-tests -- --check=$test_name; then
        echo "$test_name - ✓"
...
num_total_tests=$((num_total_tests + 1))
if timeout 2m cargo run --bin integration-tests -- --check=invalid_payload; then
...
num_total_tests=$((num_total_tests + 1))
if timeout 2m cargo run --bin integration-tests -- --check=poi_divergence_remote; then
...
num_total_tests=$((num_total_tests + 1))
if timeout 2m cargo run --bin integration-tests -- --check=poi_divergence_local; then
    echo "poi_divergence_local - ✓"
    num_success_tests=$((num_success_tests + 1))
elif [ $? -eq 124 ]; then
    echo "poi_divergence_local"
    if [[ " ${validation_tests[@]} " =~ " poi_divergence_local " ]]; then
        echo "poi_divergence_local - ✓"
        num_success_tests=$((num_success_tests + 1))
    else
        num_timeout_tests=$((num_timeout_tests + 1))
