...
RUST_BACKTRACE=1 RUST_LOG="off,hyper=off,graphcast_sdk=debug,poi_radio=trace,integration_tests=trace" cargo test --lib -- "integration_tests::checks::poi_divergence_remote::tests::test_poi_divergence_remote"
if [ $? -eq 0 ]; then
    echo "POI divergence remote test executed successfully."
else
    echo "POI divergence remote test executed with errors."
    exit 1
fi
