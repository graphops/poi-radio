...
use super::setup::constants::{MOCK_SUBGRAPH_GOERLI, MOCK_SUBGRAPH_MAINNET};
...
#[cfg(test)]
pub mod tests {
    use crate::attestation::{combine_senders, AttestationError, RemoteAttestationsMap};
    use crate::metrics::{ACTIVE_INDEXERS, INDEXER_COUNT_BY_NPOI};
    ...
    pub async fn test_process_messages(
        ...
        for msg in messages.iter() {
            ...
            let indexer_address =
                query_registry_indexer(registry_subgraph.to_string(), sender.clone())
                    .await
                    .map_err(|e| {
        ...
        let npoi_hist =
            INDEXER_COUNT_BY_NPOI.with_label_values(&[&first_msg.identifier.to_string()]);
        let blocks = remote_attestations
        ...
        let active_indexers =
            ACTIVE_INDEXERS.with_label_values(&[&first_msg.identifier.to_string()]);
