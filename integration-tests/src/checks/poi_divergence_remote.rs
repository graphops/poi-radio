        ...
        if let Some(local_attestation) = blocks.get(&block) {
            if let Some(remote_blocks) = remote.get(ipfs_hash) {
                if let Some(remote_attestations) = remote_blocks.get(&block) {
                    ...
                    // This check assumes there are 5 basic and 1 divergent instances running. This is what remote attestations looks like currently with no divergence (example):
                    // [
                    //     Attestation {
                    ...
                    if remote_attestations.len() >= 2 {
                        ...
                        info!("{}", "poi_divergence_remote test is successful ✅".green());
                        std::process::exit(0);
                    }
                }
            }
